...
namespace Mpc_diff_car {
    MPC_Controller::MPC_Controller(const MPCState &s, const nav_msgs::msg::Path &path){
        ...
        A = Eigen::MatrixXd::Zero(2 * N, 2 * N);
        l = Eigen::VectorXd::Zero(2 * N, 1);
        u = Eigen::VectorXd::Zero(2 * N, 1);
    ...
    void MPC_Controller::get_X_r_X_k(){
        int next_time_index = find_closest_trajectory_point() + 1; // 如果不加一，则返回的状态是当前状态，而不是下一状态
        if (next_time_index >= path_->poses.size() - 1) next_time_index--; // 防止溢出
    ...
    void MPC_Controller::get_H_q_A_l_u(){
        ...
        A = Eigen::MatrixXd::Identity(2 * N, 2 * N);
    ...
    void MPC_Controller::set_Q_R(){
        ...
        for (int i = 0; i < 3 * N; i += 3){
            ...
            Q(i+2, i+2) = 0.2; // 角度误差权重
        }
        for (int i = 0; i < 2 * N; i += 2){
            R(i, i) = 0.0; // 线速度惩罚权重
            R(i+1, i+1) = 0.0; // 角速度惩罚权重
        }
    }
