// 3-25 这段代码是可以跑的，就是太多没用的代码，后面可以直接拿来运行的
...
public:
    MPC_test() : Node("mpc_test") {
        ...
        timer_ = this->create_wall_timer(std::chrono::milliseconds(10), std::bind(&MPC_test::timer_callback, this));
    }
...
private:
    ...
    void timer_callback(){
        ...
        // 发布控制信号
        // std::cout << "MPC控制信号: " << cmd_vel.linear.x << " " << cmd_vel.angular.z << std::endl;
        // pub_cmd_vel_->publish(cmd_vel); // 这里控制频率比较低，后面要调整
        // 发布轨迹
        // pub_path_test_->publish(opt_traj);
        // path_test = path_test_generate();
        // double x = opt_traj.poses[path_generate_time_index].pose.position.x;
        // double y = opt_traj.poses[path_generate_time_index].pose.position.y;
        // double theta = opt_traj.poses[path_generate_time_index].pose.orientation.x;
