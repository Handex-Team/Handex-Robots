...
public:
    MPC_test() : Node("mpc_test") {
        ...
        arm_state_sub_ = this->create_subscription<std_msgs::msg::Int8>("/arm_state", 1, std::bind(&MPC_test::ArmStateCallback, this, std::placeholders::_1));
...
private:
    ...
    rclcpp::Subscription<std_msgs::msg::Int8>::SharedPtr arm_state_sub_;
    ...
    bool is_arm_state_busy = false; // 机械臂是否忙碌
    void ArmStateCallback(const std_msgs::msg::Int8::SharedPtr msg) {
        is_arm_state_busy = msg->data == 0;
    }
    void CarStatePubTimerCallback() {
        ...
        if(!is_reach_goal || is_arm_state_busy){
            car_state_msg.data = 1; // 忙碌
        }
        else{
            car_state_msg.data = 2; // 空闲
        }
