...
public:
    TrajectoryOptimizer() : Node("trajectory_optimizer"),clock(RCL_SYSTEM_TIME)
    {
        ...
        params_.max_vel = 2.0; 
        params_.max_acc = 5.0;
        params_.stable_vel = params_.max_vel / 2.0;
...
private:
    ...
    void cul_vel_xy(){
        ...
        // std::cout << "cul:"<< "vel_x: " << vel_x_ << " vel_y: " << vel_y_ << std::endl;
    }

    void cul_acc_xy()
    {
        
        // std::cout << "cul_acc_xy:" << "vel_x: " << vel_x << " vel_y: " << vel_y << std::endl;
        // 获取当前时间
        ...
        std::cout << "cul_acc_xy:" << "acc_x: " << global_vel_x << " acc_y: " << global_vel_y << std::endl;
    }
    ...
    Eigen::MatrixXd get_A_expand_matrix(const Trajectory<POLY_ORDER, DIMENSIONS>& traj,
        ...
        // A_total.block(Head_pos.rows() + Tail_pos.rows() + start_goal_vel_acc.rows() + V_cont.rows(), 
        //                         0, v_max_min.rows(), num_segments * matrix_size + 2) = v_max_min;
        // A_total.block(Head_pos.rows() + Tail_pos.rows() + start_goal_vel_acc.rows() + V_cont.rows() + v_max_min.rows(), 
        //                         0, a_max_min.rows(), num_segments * matrix_size + 2) = a_max_min;
    ...
    {   
        ...
        if(!isSolutionValid(solution_x) ||!isSolutionValid(solution_y)){
            ...
            // std::cout <<"Q:\n" << Q << std::endl;
            // std::cout << "A_expand_x:\n" << A_expand_x << std::endl;
            // std::cout << "path_x[0]" << path_x[0] << std::endl;
            // std::cout << "path_x[end]" << path_x[num_points-1] << std::endl;
            // std::cout << "solution_x:\n" << solution_x.transpose() << std::endl;
            // std::cout << "state_x:" << state_x.vel << " " << state_x.acc << std::endl;
        }
    ...
    void publishOptimizedTrajectory(const Trajectory<POLY_ORDER, DIMENSIONS> &traj)
    {
        ...
        path_msg.header.frame_id = "map"; // 确保坐标系正确"轨迹时间：%f, 轨迹点：%f, %f, 速度：%f, 角速度：%f, 偏航角：%f", 
